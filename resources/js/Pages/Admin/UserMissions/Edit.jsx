...
export default function EditUserMission({usermission,users,missions}) {
    const [filteredUsers, setFilteredUsers] = useState([]);
    const [userSearchTerm, setUserSearchTerm] = useState(`${usermission.user_id}`);
    ...
    const { data, setData, post, errors, processing } = useForm({
        ...
        reward_claimed: usermission.reward_claimed || 0,
    });

    useEffect(() => {
        const selectedUser = users.find(u => u.userID === usermission.user_id);
        if (selectedUser) {
            setUserSearchTerm(`${selectedUser.userID} - ${selectedUser.memberID}`);
        }
    }, [usermission.user_id, users]);
    ...
    const submit = (e) => {
        ...
        post(`/admin/usermissions/${usermission.id}`, formData, {
            forceFormData: true
        });
    };
    ...
    return (
        <AdminLayout
            title="Edit User Mission"
            breadcrumbs={[
                { label: "Admin", url: "/admin" },
                { label: "User Missions", url: "/admin/user-missions" },
                { label: "Edit User Mission" }
            ]}
        >
            <Head title="Edit User Mission" />
            <div className="p-4 bg-white shadow-md rounded-lg">
                <form className="flex flex-col gap-4" onSubmit={submit}>
                    ...
                    <table className="table">
                        ...
                        <tbody>
                            <tr>
                                <td>{usermission.user_id}</td>
                                <td>{usermission.mission_id}</td>
                                <td>{usermission.progress}</td>
                                <td>{usermission.reward_claimed}</td>
                            </tr>
                        </tbody>
